...
class ConstraintSatisfaction:
    ...
    def selectVariable(solutionObj, variables, choosableVars, orderedVars,
                        originalAssignment, timeWindowViolation, currentRoute,
                        level, currentTime, constraints):
        if timeWindowViolation is not None:
            ...
            originalAssignment = False
            idx = None
            # heuristic #1: from those which muss come after this one, take the closest one
            if level > 0:
                constraintsLeftSide = constraints[:,0]
                possibleStopsIdx = numpy.argwhere(constraintsLeftSide == currentRoute[level-1])
                possibleStops = set(constraints[:,1][possibleStopsIdx].flat) & set(choosableVars)
                if len(possibleStops) == 1:
                    idx = choosableVars.index(possibleStops.pop())
                elif len(possibleStops) > 1:
                    minPossibleNextTime = None
                    minPossibleNext = None
                    for nextStop in possibleStops:
                        nextTime = max(currentTime + solutionObj.getTimeDistance(currentRoute[level-1], nextStop),
                            solutionObj.getWindowOf(nextStop)[0])
                        if minPossibleNextTime is None or nextTime < minPossibleNextTime:
                            minPossibleNextTime = nextTime
                            minPossibleNext = nextStop
                    idx = choosableVars.index(minPossibleNext)
            # heristic #2: pick the lowest in time window
            if idx is None:
    ...
    def satisfyConstraints(solutionObj, variables, domains, constraints, orderedVars,
        ...
        currentCapacity=0, currentTime=0., timeWindowViolation=None, boardTime={}):
        ...
        while len(choosableVars)>0:
            ...
            if pickedVar is not None:
                ...
                if newCurrentTime <= maxT:
                    ...
                    if consistent and (pickedVar + solutionObj.totalRequests) in boardTime:
                        consistent = (currentTime - boardTime[pickedVar + solutionObj.totalRequests] - waitingTime) <= solutionObj.requestGraph.userTime
                    if consistent:
                        ...
                        if len(variables) > 1:
                            ...
                            boardTime[pickedVar] = newCurrentTime - waitingTime
                            ...
                            newDomains,timeWindowViolation = ConstraintSatisfaction.satisfyConstraints(solutionObj,
                                ...
                                newCapacity, newCurrentTime, timeWindowViolation, boardTime)

                            # if this assignment succeeded, break the loop
                            if newDomains is not None:
                                break

                            del boardTime[pickedVar]
                        else:
