...
param Lim_max {V} >= 0;
param Q {V} >= 0;								# Load of the request
...
subject to S6_1 {(i,j) in EDGE, b in BUS}:
	w[j,b] >= w[i,b] + Q[j] - min(Cap[b], Cap[b] + Q[i])*(1 - x[i,j,b]);
